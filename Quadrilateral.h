...
public:
	...
	Quadrilateral(const Point& a , const Point& c, const Color& color);

	// Verilen en ve boy deðerlerine uygun olarak bir dikdörtgen/kare oluþturur a her zaman sol üst noktayý gösterir
	Quadrilateral(const Point& a, double width, double height, const Color& color);

	// Verilen noktalara göre dörtgen oluþturur. Bu noktalarýn bir dörtgen oluþturduðu kontrol edilmeldir.
	Quadrilateral(const Point& a, const Point& b, const Point& c, const Point& d, const Color& color);
	Quadrilateral(const array<Point, NUMBER_OF_CORNERS>& pts, const Color& color);
